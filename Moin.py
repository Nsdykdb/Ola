...
bot = telebot.TeleBot('8031510326:AAHWNQ342W4pH97eMefegHD64M1RsrFNnik')

# Admin user IDs
admin_id = ["YOUR_ADMIN_ID_HERE", "6485336955"]
...
def welcome_start(message):
    ...
    # Second button row
    keyboard.row(
        telebot.types.InlineKeyboardButton('🎓 How To Use', url='https://t.me/Vip_Ddos_07'),
...
def validate_and_execute_attack(message, target, port, time):
    ...
    if len(command) > 0:
        ...
        if validate_key(user_key):
            ...
            keyboard = telebot.types.InlineKeyboardMarkup()
            join_button = telebot.types.InlineKeyboardButton('🚩 Join Channel 🚩', url='https://t.me/lordbuyhackof')
